...
%% FEATURES:
% Retrain a previously trained agent
% Full actuation of the Arms
% A fully interactable box
% modified RL goal to push box
...
CH_humanoid_walker_rl_parameters
%% 
% Select model.

mdlName='CH_humanoid_walker_rl';
load_system(mdlName);
...
numAct          = 6+8; % 6 leg joints, 8 Arm joints
...
numObs               = 10 + 12 + 16 + 1 + numAct;
% 10 Torso Position sensors, 12 Leg joint wheel decoders, 16 Arm Joint wheel decoders, Box vision, Previous actions
...
elseif PriorAgent == true
    agent = load('Agent1888');
end
...
trainingOptions.MaxEpisodes                = 6000;
...
trainingOptions.StopOnError                = 'on';
...
if trainWalker
    ...
    saveDir = 'savedAgents';
    cd(saveDir)
    save(['humanoid_walker_rl_trained_' datestr(now,'DD_mmm_YYYY_HHMM')],'agent','trainingResults');
    cd(curDir)
